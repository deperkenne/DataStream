...
from pyspark.sql.types import (StructType,
                               ...
                               TimestampType, Row)
from sparkstream.repo_voting import insertdatatotables
...
if __name__ == "__main__":
    ...
    def Kafka_connection():
        return KafkaProducer(bootstrap_servers=['192.168.178.194:9092'],
    ...
    def choice_candidate():
        vote = 1
        candidates_data = get_data_from_db(
            url,
            table_name_candidates,
            properties
        ).collect()
        voters_data = get_data_from_db(
            url,
            table_name_voters,
            properties
        ).collect()
        ...
        for voter in voters_data:
            ...
            send_data_to_Kafka_topic(
                ...
                candidate_id=random_choice["candidate_id"],
                candidate_name=random_choice["candidate_name"],
                party_affiliation=random_choice["party_affiliation"],
                candidate_picture=random_choice["photo_url"],
                vote_time=voting_time_str
            )

            insertdatatotables.save_data_to_table_vote(
                conn, cur,
                voter_id=voter["voter_id"],
                candidate_id=random_choice["candidate_id"],
